...
/* ============================================================
   frontend/api.js  — complete replacement
   ============================================================ */
...
const AdminAPI = {
  listUsers(role) {
    return apiFetch(`/api/admin/users${role ? "?role=" + role : ""}`);
  },
  listStudents() {
    return apiFetch("/api/admin/users/students");
  },
  updateRole(id, role) {
  ...
  toggleActive(id) {
    return apiFetch(`/api/admin/users/${id}/activate`, { method: "PATCH" });
  },
  deleteUser(id) {
...
/* ── Enrollment API (teacher/admin) ─────────────────────────────────────────── */
const EnrollAPI = {
  enroll(quizId, userIds) {
    return apiFetch(`/api/quizzes/${quizId}/enroll`, {
      method: "POST",
      body: JSON.stringify({ user_ids: userIds }),
    });
  },
  removeEnrollment(quizId, uid) {
    return apiFetch(`/api/quizzes/${quizId}/enroll/${uid}`, {
      method: "DELETE",
    });
  },
  getStudents(quizId) {
    return apiFetch(`/api/quizzes/${quizId}/students`);
  },
};
...
const UI = {
  ...
  applyRBAC() {
    ...
    // Hide Create Quiz button for students
    document
      .querySelectorAll('a[href="create-quiz.html"], #create-quiz-btn')
      .forEach((el) => {
        if (isStudent) el.style.display = "none";
      });
    // Hide Create Quiz sidebar item for students
    document.querySelectorAll(".menu-item").forEach((el) => {
      if (isStudent && el.href && el.href.includes("create-quiz.html")) {
